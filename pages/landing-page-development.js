...
function LandingPageDevelopment({ heroData, ourWork, seoData, error }) {
	...
	useEffect(() => {
		const APIurl = `${process.env.NEXT_PUBLIC_API_URL}/api/landing-page-development`;
...
export async function getStaticProps() {
	...
	let newUrl = `${process.env.NEXT_PUBLIC_API_URL}/api/landing-page-development`;
	...
	try {
		...
		// let keyserveicedata = await fetch(`${URL}/api/keyserveice?slug=landingPageDevelopment`);
		// let keyServeice = await keyserveicedata.json();

		// let faqSectiondata = await fetch(`${URL}/api/faqsection?slug=landingPageDevelopment`);
		// let faqData = await faqSectiondata.json();

		// let ourWorkData = await fetch(`${URL}/api/ourwork?slug=landingPageDevelopment`);
		// let ourWork = await ourWorkData.json();

		// let listOfServeice = await fetch(`${URL}/api/listofserveice?slug=landingPageDevelopment`);
		...
		return {
			props: { heroData, ourWork, seoData }, // will be passed to the page component as props
			revalidate: 60,
		};
	} catch (error) {
