...
function WebsiteDevelopment({ heroData, ourWork, seoData, error }) {
	...
	useEffect(() => {
		const APIurl = `${process.env.NEXT_PUBLIC_API_URL}/api/website-development`;
...
export async function getStaticProps() {
	let newUrl = `${process.env.NEXT_PUBLIC_API_URL}/api/website-development`;
	...
	try {
		...
		// let keyserveicedata = await fetch(`${URL}/api/keyserveice?slug=WebsiteDevelopment`);
		// let keyServeice = await keyserveicedata.json();

		// let faqSectiondata = await fetch(`${URL}/api/faqsection?slug=WebsiteDevelopment`);
		// let faqData = await faqSectiondata.json();

		// let ourWorkData = await fetch(`${URL}/api/ourwork?slug=WebsiteDevelopment`);
		// let ourWork = await ourWorkData.json();

		// let listOfServeice = await fetch(`${URL}/api/listofserveice?slug=WebsiteDevelopment`);
