...
function ClientCall({ contactData, seoData }) {
	...
	useEffect(() => {
		...
		const APIurl = `${process.env.NEXT_PUBLIC_API_URL}/api/schedule-a-call`;
...
export async function getStaticProps(context) {
	...
	try {
		...
		let data = await fetch(`${URL}/api/schedule-a-call?populate=*`);
