...
function MicrosoftAds({ heroData, seoData, ourWork, error }) {
	...
	useEffect(() => {
		const APIurl = `${process.env.NEXT_PUBLIC_API_URL}/api/microsoft-ad`;
...
export async function getStaticProps() {
	let newUrl = `${process.env.NEXT_PUBLIC_API_URL}/api/microsoft-ad`;
	try {
		...
		// let keyserveicedata = await fetch(`${URL}/api/keyserveice?slug=semAgency`);
		// let keyServeice = await keyserveicedata.json();

		// let faqSectiondata = await fetch(`${URL}/api/faqsection?slug=semAgency`);
		// let faqData = await faqSectiondata.json();

		// let ourWorkData = await fetch(`${URL}/api/ourwork?slug=semAgency`);
		// let ourWork = await ourWorkData.json();

		// let listOfServeice = await fetch(`${URL}/api/listofserveice?slug=semAgency`);
