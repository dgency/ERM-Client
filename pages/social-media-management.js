...
function SocialMediaManagement({ heroData, ourWork, seoData, error }) {
	...
	useEffect(() => {
		const APIurl = `${process.env.NEXT_PUBLIC_API_URL}/api/social-media-management`;
...
export async function getStaticProps() {
	let newUrl = `${process.env.NEXT_PUBLIC_API_URL}/api/social-media-management`;
	try {
		...
		// let keyserveicedata = await fetch(`${URL}/api/keyserveice?slug=SocialMediaManagement`);
		// let keyServeice = await keyserveicedata.json();

		// let faqSectiondata = await fetch(`${URL}/api/faqsection?slug=SocialMediaManagement`);
		// let faqData = await faqSectiondata.json();

		// let ourWorkData = await fetch(`${URL}/api/ourwork?slug=SocialMediaManagement`);
		// let ourWork = await ourWorkData.json();

		// let listOfServeice = await fetch(`${URL}/api/listofserveice?slug=SocialMediaManagement`);
