...
function LinkedInAds({ heroData, companiesData, ourWork, seoData, fineWebsiteData, error }) {
	...
	useEffect(() => {
		const APIurl = `${process.env.NEXT_PUBLIC_API_URL}/api/linked-in-ad`;
		...
		return (
			<div>
				...
				<ListOfServeice
					...
					mobileImage="linkBuildingService/escaperoom-marketer-link-building-services-list.svg"
				/>
				<div className="break_line image bg-[url('/paper-cropped2.svg')]  h-[50px] w-full bg-repeat bg-[length:1800px_100px] bg-center  "></div>
				<CaseStudySection
...
export async function getStaticProps(context) {
	...
	let newUrl = `${process.env.NEXT_PUBLIC_API_URL}/api/linked-in-ad`;
	try {
		...
		// let cardContent = await fetch(`${URL}/api/cardcontent?slug=LinkedInAds`);
		...
		return {
			props: { companiesData, heroData, seoData, ourWork, fineWebsiteData }, // will be passed to the page component as props
			revalidate: 60,
		};
	} catch (error) {
